...
type monadic = (int * Term.types * Term.constr)

let pr_monadic monadic =
  let (purelevel, ty, term) = monadic in
  hv 0 (str "monadic" ++ int purelevel ++ spc () ++ Printer.pr_constr ty ++ spc () ++ Printer.pr_constr term)
...
let monadic_is_function m =
  Term.isProd (rawtype_of_monadic m)

let monadic_is_value m = not (monadic_is_function m)

let rec numargs_of_type ty =
  match Term.kind_of_term ty with
  | Term.Prod (name, ty', body) ->
      1 + numargs_of_type body
  | _ -> 0

let rec prod_appvect ty args =
  let numargs = numargs_of_type ty in
  if numargs = 0 then
    error "Not enough prod's."
  else if Array.length args <= numargs then
    Term.prod_appvect ty args
  else
    let args1 = Array.sub args 0 numargs in
    let args2 = Array.sub args numargs (Array.length args - numargs) in
    prod_appvect (Term.prod_appvect ty args1) args2

let pr_explain_monadic m =
  let (purelevel, rawty, term) = m in
  let numargs = numargs_of_type rawty in
  (if numargs < purelevel then
    str "is pure"
  else
    str "=>" ++ spc () ++ Printer.pr_constr term) ++
  spc () ++ str "(purelevel=" ++ int purelevel ++ str ")"

let monadic_is_pure m =
  let (purelevel, ty, term) = m in
  numargs_of_type ty < purelevel
...
let delete_univ env evdref term =
  let rec recfun term =
    ...
    | Term.Evar (ekey, termary) -> Term.mkEvar (ekey, (Array.map recfun termary))
    | Term.Sort s ->
        (match s with
        | Sorts.Prop _ -> term
        | Sorts.Type _ -> Evarutil.e_new_Type env evdref)
    | Term.Cast (expr, kind, ty) -> Term.mkCast (recfun expr, kind, recfun ty)
    | Term.Prod (name, ty, body) -> Term.mkProd (name, recfun ty, recfun body)
    | Term.Lambda (name, ty, body) -> Term.mkLambda (name, recfun ty, recfun body)
    | Term.LetIn (name, expr, ty, body) -> Term.mkLetIn (name, recfun expr, recfun ty, recfun body)
    | Term.App (f, argsary) -> Term.mkApp (recfun f, Array.map recfun argsary)
    | Term.Const (cnst, u) -> Term.mkConst cnst
    | Term.Ind (ind, u) -> Term.mkInd ind
    | Term.Construct (cstr, u) -> Term.mkConstruct cstr
    | Term.Case (ci, tyf, expr, brs) -> Term.mkCase (ci, recfun tyf, recfun expr, Array.map recfun brs)
    | Term.Fix ((ia, i), (nameary, tyary, funary)) ->
        Term.mkFix ((ia, i), (nameary, Array.map recfun tyary, Array.map recfun funary))
    | Term.CoFix (i, (nameary, tyary, funary)) ->
        Term.mkCoFix (i, (nameary, Array.map recfun tyary, Array.map recfun funary))
    | Term.Proj (proj, expr) ->
        Term.mkProj (proj, recfun expr)
  in
...
let mona_return_notset : Term.constr option = None
...
let mona_bind_notset : Term.constr option = None
...
let mona_action_add libref constr =
  ...
  let pureterm =
    match gref with
    | ConstRef cnst -> Term.mkConst cnst
    | ConstructRef cstr -> Term.mkConstruct cstr
    | _ -> error "unexpected gref"
  in
  let termty = type_of env evdref pureterm in
  let purelevel = numargs_of_type termty in
...
let mona_type_notset : Term.constr option = None
...
let mona_type0 ty =
  ...
  | Some ret -> Term.mkApp (ret, [| ty |])

let rec convert_type pure_level ty =
  if pure_level = 0 then
    match Term.kind_of_term ty with
    | Term.Prod (argname, argty, bodyty) ->
        mona_type0 (Term.mkProd (argname, convert_type 1 argty, convert_type 0 bodyty))
    | _ -> mona_type0 ty
  else
    match Term.kind_of_term ty with
    | Term.Prod (argname, argty, bodyty) ->
        Term.mkProd (argname, convert_type 1 argty, convert_type (pure_level-1) bodyty)
    | _ -> ty

let rec monadify_type purelevel ty =
  ...
  | Term.Prod (name, namety, body) ->
      if purelevel = 0 then
        mona_type0 (Term.mkProd (name, monadify_type 1 namety, monadify_type 0 body))
      else
        Term.mkProd (name, monadify_type 1 namety, monadify_type (purelevel - 1) body)
  | Term.Sort _ | Term.Rel _ | Term.Ind _ ->
      wrap_type ty
  | Term.App (f, args) ->
      wrap_type (match Term.kind_of_term f with
      | Term.Ind (ind, u) ->
          Term.mkApp (Term.mkIndU (ind, u), Array.map (monadify_type 1) args)
      | _ ->
          (Feedback.msg_warning (hv 0
            (str "monadify_type: unexpected type application:" ++ spc () ++ Printer.pr_constr ty));
            ty))
  | _ ->
      (Feedback.msg_warning (hv 0
        (str "monadify_type: unexpected type:" ++ spc () ++ Printer.pr_constr ty));
      wrap_type ty)

let mona_return0 ty term =
  match !mona_return_ref with
  | None -> raise (MonadificationError "Monadify Return not set")
  | Some ret -> Term.mkApp (ret, [| ty; term |])

let mona_bind0 ty1 ty2 term1 term2 =
  match !mona_bind_ref with
  | None -> raise (MonadificationError "Monadify Bind not set")
  | Some bind -> Term.mkApp (bind, [| ty1; ty2; term1; term2 |])

(* puredown doesn't convert types. *)
let rec puredown j m =
  let (i, rawtermty, term) = m in
  if i < j then
    errorlabstrm "puredown"
      (hv 0 (str "puredown: cannot up purelevel:" ++ spc () ++ pr_monadic m ++ spc () ++ hv 0 (str "to" ++ spc () ++ int j)))
  else if i = j then
    term
  else (* 0 <= j < i *)
    match Term.kind_of_term rawtermty with
    | Term.Prod (argname', argty', bodyty) ->
        (match Term.kind_of_term term with
        | Term.Lambda (argname, argty, body) ->
            if j = 0 then
              let body' = puredown 0 (i-1, bodyty, body) in
              mona_return0 (monadify_type 1 rawtermty) (Term.mkLambda (argname, argty, body'))
            else
              let body' = puredown (j-1) (i-1, bodyty, body) in
              Term.mkLambda (argname, argty, body')
        | _ ->
            (* This eta-expansion should not delay side effect
               because 0 < i which means that e has no immediate side effect. *)
            let body' = Term.mkApp (Vars.lift 1 term, [| Term.mkRel 1 |]) in
            puredown j (i, rawtermty, Term.mkLambda (Name.Anonymous, argty', body')))
    | _ ->
        mona_return0 (monadify_type 1 rawtermty) term

let puredown' j m =
  let (i, rawtermty, term) = m in
  (j, rawtermty, puredown j m)

let rec pureapprox term =
  match Term.kind_of_term term with
  | Term.Lambda (name, ty, body) ->
      1 + pureapprox body
  | Term.Fix ((ia, i), (nameary, tyary, funary)) ->
...
let define_constant id term =
  ...
  Declare.declare_definition id (term, Evd.universe_context_set !evdref)
  ...
  | Term.App (f, args) ->
      ...
      | _ ->
          (Feedback.msg_warning (hv 0
            ...
            spc () ++ Printer.pr_constr_env env !evdref ty));
          false))
  | _ ->
      (Feedback.msg_warning (hv 0
...
let mona_pure_def gref =
  ...
  let numargs = numargs_of_type termty in
...
let mona_bind2_internal name m1 m2 =
  let (purelevel1, rawty1, term1) = m1 in
  let (purelevel2, rawty2, term2) = m2 in
  let rawty = Term.prod_appvect (Term.mkProd (name, rawty1, rawty2)) [| term1 |] in
  if Term.isRelN 1 term2 then
    m1
  else if 0 < purelevel1 then
    (purelevel2, rawty,
      if Term.isRel term1 || Termops.count_occurrences (Term.mkRel 1) term2 <= 1 then
        Reduction.beta_app (Term.mkLambda (name, (monadify_type 1 rawty1), term2)) (puredown 1 m1)
      else
        Term.mkLetIn (name, (puredown 1 m1), (monadify_type 1 rawty1), term2))
  else
    (0, rawty,
      mona_bind0 (monadify_type 1 rawty1) (monadify_type 1 (Vars.lift (-1) rawty2))
        term1
        (Reductionops.shrink_eta (Term.mkLambda (name, (monadify_type 1 rawty1),
          (puredown 0 m2)))))

let mona_bind2 name m1 m2 =
  let result = mona_bind2_internal name m1 m2 in
...
let bind_mctx mctx mterm =
  List.fold_left (fun mterm (name, marg) -> mona_bind2 name marg mterm) mterm mctx
...
let mona_construct_ref env evdref (cstr, u) =
  ...
  if List.mem_assoc key !mona_record_ref then
    ...
    Feedback.msg_info (hv 0 (str "constructor found:" ++ spc () ++ Printer.pr_constructor env cstr ++ spc () ++ pr_explain_monadic m));
    m)
  else
...
let pr_head env evdref mctx mterm =
  ...
  let ppcmds_mctx, env2, _ = List.fold_right
    (fun (name, mctx_elt) (prs, e, i) ->
      ...
      let decl = Context.Rel.Declaration.LocalAssum (name, convert_type 1 (rawtype_of_monadic mctx_elt)) in
      let e2 = Environ.push_rel decl e in
      (pr::prs, e2, i - 1))
    mctx
    ([], env, n)
  in
...
let make_purelevel_positive (mctx, mterm) =
  ...
  if purelevel = 0 then
    ((Name.Anonymous, mterm) :: mctx, (1, Vars.lift 1 rawty, Term.mkRel 1))
  else
    (mctx, mterm)
...
let rec mona_const_ref env evdref (cnst, u) =
  ...
  if List.mem_assoc key !mona_record_ref then
    (let (converted, m) = List.assoc key !mona_record_ref in
    Feedback.msg_info (hv 0 (str "constant found:" ++ spc () ++ Printer.pr_constant env cnst ++ spc () ++ pr_explain_monadic m));
    m)
  else
    ...
    let (term0, uconstraints) =
      ...
      | None -> errorlabstrm "mona_const_ref"
    ...
    if mona_pure_dependencies_p env evdref term && not higher_order_p then
      ...
      (if !opt_verbose then
        Feedback.msg_info (hv 2 (hv 0 (str "monadification generated:" ++ spc () ++ Id.print id ++ spc () ++ str ":=") ++ spc () ++ Printer.pr_constr_env env !evdref term)));
      ...
      let v = Term.mkConst constant in
...
and mona_pure_dependencies_p env evdref term =
  ...
  let rec recfun env term =
    ...
    | Term.Const (cnst, u) ->
        translated := mona_const_ref env evdref (cnst, u) :: !translated
    | Term.Ind (ind, u) -> ()
  ...
  in
  (recfun env term;
  List.for_all monadic_is_pure !translated)
...
and mona_head_internal env evdref rel_purelevels term =
  ...
  if Term.isSort termty then
    ...
    | Term.Cast (expr, kind, castty) ->
        ...
        ((Name.Anonymous, m) :: mctx,
         (1, Vars.lift (nb+1) termty,
           Term.mkCast (Term.mkRel 1, kind, Vars.lift (nb+1) castty)))
    ...
    | Term.App (f, argary) ->
        ...
        in
        let args1 = Array.map (puredown 1) margs1 in
        let rawty1 = prod_appvect mf2_rawty rawargs1 in
        let mterm1 = (mf2_purelevel - Array.length margs1, rawty1, Term.mkApp (mf2_term, args1)) in
        let (mctx, mterm, rawty) =
          Array.fold_left
            (fun (mctx, mterm, rawty) i ->
              ...
              let args2 = [| puredown 1 lifted_marg |] in
              let rawty2 = Vars.lift (i+1) (Term.prod_appvect rawty [| lifted_rawarg |]) in
              let mterm2 = (0, rawty2, Term.mkApp (Term.mkRel 1, args2)) in
              (mctx2, mterm2, rawty2))
            (mctx, mterm1, rawty1)
    ...
    | Term.Case (ci, tyf, expr, brs) ->
        let (name, exprty, bodyty) = Term.destLambda tyf in
          ...
          array_map2
            (fun numargs br_mterm ->
              (if purelevel_of_monadic br_mterm < numargs then
                ...
                error "too small purelevel of match branch");
        ...
        let purelevel = Array.fold_left
          ...
          br_purelevels
        in
        let mtyf = Term.mkLambda (name, exprty, monadify_type purelevel bodyty) in
        let brs' =
          array_map2
            (fun numargs br_mterm ->
              puredown (numargs + purelevel) br_mterm)
            cstr_numargs
            translated_brs
        in
        make_purelevel_positive
          ((Name.Anonymous, mexpr) :: mctx_expr,
           (purelevel,
            ...
            Term.mkCase (ci, Vars.lift (n+1) mtyf, Term.mkRel 1, (Array.map (Vars.lift (n+1)) brs'))))
    ...
    | Term.Lambda (name, namety, body) ->
        ...
        ([],
         (body_purelevel + 1,
          termty,
          Term.mkLambda (name, monadify_type 1 namety, body')))

    | Term.Fix ((ia, i), (nameary, tyary, funary)) ->
        let env2 = Environ.push_rec_types (nameary, tyary, funary) env in
        let approx_purelevels = Array.map pureapprox funary in
        ...
        let tyary2 = array_map2
          (fun f_purelevel ty -> monadify_type f_purelevel ty)
          approx_purelevels
          tyary
        in
        let funary2 = Array.map2 (fun mfun i -> puredown i mfun) mfunary approx_purelevels in
        ([],
         (approx_purelevels.(i),
          termty,
          Term.mkFix ((ia, i), (nameary, tyary2, funary2))))
...
let monadification_single libref =
  ...
  | ConstructRef cstr ->
      ...
      ()
  | _ -> error "not constant or constructor"
